...
def main() -> None:
    ...
    if not pending:
        ...
        # Load last checkpoint to close any storms still active at end of record
        last_done = max(done)
        state = load_state(last_done)
        if state and state.get("active_storms"):
            tail_storms = state["active_storms"]
            log.info("Closing %d storms still active at end of record.", len(tail_storms))
            tail_events: list[dict] = []
            for event_id, storm in tail_storms.items():
                rp = classify_storm_rp(storm, atlas14_grids)
